...
**Tokens and Components.** The code of the system is `design/system/`, a package that can be lifted whole; the showcase and the documents are `design/kit/`, and the roadmap points at `design/kit/overview.html` because a sidebar shows what can be opened. Two levels of token and not three: primitive holds values, semantic holds roles, and a component token is created point by point where a state lands on no role. **Colour is read only through a role, geometry straight from primitive** - a theme replaces colour, and a radius has nothing to override. **A role and a state token live in both themes or they do not exist,** and the pair is not a mirror: it takes another step of the ramp. **Adding a component is five things:** a css file, a page, a row in the stand registry, a row in the inventory with its level, and an `@import` in the group of **its own level, never the end of the file**. Levels are counted by nesting and never grouped by purpose, because four things read that column. Detail lives in `design/system/CLAUDE.md`, which loads by itself inside that folder, and the reasoning in `design/kit/docs/architecture.md`.

**Concept onward.** Colour never lands on the grey wireframes: `design/` holds coloured copies that own the visual layer only, while structure, text and the set of states stay the property of `wireframes/`. A copy that needs to know what kind of screen it is on asks markup that already exists, never a class added for the occasion; icons arrive through CSS, not through a new element. **Every colour, shape and typeface decision has a pair in the attributes or the recorded designer taste, or it is invention,** and a value read off a generated image is taken from its pixels, never from the caption printed on it. A reference is an input, never an output. A palette that can be guessed from the category is a model reflex, not a decision. Values move forward and are never re-derived. **One file holds a value at a time**, and it has moved twice by `git mv` with the values untouched: `design/_theme.css`, then `design/kit/kit.css`, and since stage 08 `design/system/tokens.css`. `DESIGN-artifacts.md` and `DESIGN.md` are its written records and the stands are its live ones; the records disagreeing is a defect, not a variation.
...
**"We keep it" is one word, and it means four addresses.** A value goes into the token of its own level in `design/system/tokens.css`, colour into a semantic role and geometry into a primitive, and it reaches every screen by itself. Markup goes onto the component page and onto the screens the component stands on. The record syncs in `DESIGN.md`, and the attribute is signed in `design/concept/docs/concept.md`; if the fix contradicts an attribute, change the attribute and say so out loud. Then check in the browser that it arrived everywhere by itself. A value carried by hand is a second copy, and drift starts from exactly there.
...
| Code of the design system | `design/system/`, rules in `design/system/CLAUDE.md` |
| System rules, inventory, audits, backlog | `design/kit/docs/` |
